...
export const Footer = ({ data }: { data: PageBlocksFooter }) => {
  ...
  return (
    <motion.footer
      ref={ref}
      className="flex justify-between rounded-3xl bg-black h-[83px] w-[95%] mx-auto my-3 mb-4 text-white text-base flex-wrap xs:mb-6 max-[640px]:flex-col max-[640px]:h-auto max-[640px]:p-4"
    >
      <motion.div
        className="grid place-items-center mx-5 max-[640px]:mb-4"
        ...
        {data.logo && (
          ...
          >
            <Image
              ...
              className="transition-transform duration-200 max-[640px]:w-12 max-[640px]:h-12"
            />
          </button>
        )}
      </motion.div>
      <div className="flex flex-col justify-end p-[20px] max-[640px]:items-center max-[640px]:p-2">
        ...
        <div>
          <p
            ...
            className="max-[640px]:text-center max-[640px]:text-sm"
          >
            {data.text ||
